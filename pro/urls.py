...
def set_p_count():
    ...
    p_p_count = tbl_page_data.objects.filter(status_process=tbl_page_data.PROCESSING_STATUS).count()
    p_n_count = tbl_page_data.objects.filter(status_process=tbl_page_data.NONE_STATUS).count()
