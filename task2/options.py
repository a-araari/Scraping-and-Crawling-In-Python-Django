...
def decrease_all_pending_tasks_count(min_p):
    try:
        for tbl in tbl_crawl_task.objects.filter(pending_task_gt=min_p):
            try:
...
def _start_crawl_task(tbl):
    ...
    while t < 3:
        try:
            ...
            t += 1
            tbl.save()
            decrease_all_pending_tasks_count(tbl.pending_task)
